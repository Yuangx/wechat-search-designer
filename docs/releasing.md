...
版本 `0.2.0-rc.1`。公开候选已经移除第三方 PDF、AI、字体及品牌图片，使用者通过设计输入提供本地标识。演示使用清楚标记的原创占位图。

源码仓库：[Yuangx/wechat-search-designer](https://github.com/Yuangx/wechat-search-designer)。代码许可证尚待维护者确认，公开可见不代表已授予开源使用许可。候选源码上传、Actions 检查和 Tag / Release 是分别记录的状态；远端检查结果以仓库 Actions 页面为准。

## 发布前

1. 确定代码许可证与版权所有者，在根目录加入完整 `LICENSE`，同步 README、NOTICE、CONTRIBUTING、package.json、package-lock.json 和 SKILL frontmatter 的许可状态；将 LICENSE 加入分发清单。第三方素材仍单独适用其条件。
...
6. 远端检查通过后再创建对应 Tag / Release，上传 ZIP 与 manifest。将此版本标为 prerelease，直到支持范围满足正式发布预期。
...
GitHub 的 README、CONTRIBUTING、CODE_OF_CONDUCT、SECURITY 与 Issue 模板是帮助使用和协作的项目材料，不能把它们全部称为 GitHub 强制要求。许可证决定使用授权，结构检查通过不能代替维护者的许可证选择。
