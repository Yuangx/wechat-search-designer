...
源码版本 `0.3.0`，采用 [MIT 许可证](../LICENSE)，版权所有者为 Yuangx。公开包不附第三方 PDF、AI、字体及品牌图片，含搜索模块时，使用者通过设计输入提供本地标识。演示使用清楚标记的原创占位图。
...
2. 运行 `npm ci`、`npm run check`、`npm test`；用原有三类示例和组合图示例查看 PNG、SVG 与手机图，验证用户尺寸保持不变。真实品牌标识只作本地测试输入，不进入公开目录。
...
- Description：`Article-aware author, WeChat Search and engagement card design with local PNG, SVG and HTML export.`
